...
class Orientation extends CustomEvent {
	...
	init( Main, $window = $( window ), xAmp = this.X_AMP, yAmp = this.Y_AMP, needDispatchEvent = false, debug = this.DEBUG ) {
	...
	_bindEvents( Main ) {
		...
		Screen.bind( Screen.E.RESIZE, this._resize, this );
	...
	_raf() {
		this.valuesI.x = Math_.getInertia( this.values.x, this.valuesI.x, this.GYROSCOPE_INERTIA, false );
		this.valuesI.y = Math_.getInertia( this.values.y, this.valuesI.y, this.GYROSCOPE_INERTIA, false );
		this.valuesI.z = Math_.getInertia( this.values.z, this.valuesI.z, this.GYROSCOPE_INERTIA, false );
	}
	
	
	_onOrientationChange() {
		// if ( STF.Configs.Device.IS_DESKTOP || this.prevbW == STF.Controllers.Screen.bW )
		if ( this.prevbW == Screen.bW )
			return;
		
		
		this.prevbW = Screen.bW;
		
		if ( Screen.wH > Screen.bW ) {
